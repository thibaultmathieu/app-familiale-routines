...
**Reward system**: Each child draws from the **union of all owned universes** (`unlockedUniverseIds`) — a child who has unlocked several universes gets images from all of them at random. `unlockReward()` in `useAppState` computes the pick BEFORE `setState` (deterministic return value) and picks a random not-yet-unlocked image from the combined pool; when every image across all owned universes has been collected, it clears those ids from `unlockedImages` and increments `completedCycles`. Per-universe progress is still the intersection `unlockedImages ∩ pool`, so the gallery shows one filling section per owned universe.
